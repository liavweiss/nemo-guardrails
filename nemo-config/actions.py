# Input + output rails: keyword and pattern checks, no LLM, no API key.
import re
...
def check_harmful_keywords(text: str) -> bool:
    """Return True if the message should be blocked (harmful). No LLM."""
...
def check_sensitive_patterns(text: str) -> bool:
    """Return True if the message looks like sharing secrets/PII. No LLM."""
    if not text or not isinstance(text, str):
        return False
...
def check_output_harmful_keywords(text: str) -> bool:
    """Return True if the assistant message should be blocked (harmful). No LLM."""
    if not text or not isinstance(text, str):
        return False
    lower = text.lower().strip()
    return any(phrase in lower for phrase in BLOCKED_PHRASES)
